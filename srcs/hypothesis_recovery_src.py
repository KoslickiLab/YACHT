...
def get_organisms_with_nonzero_overlap(manifest, sample_file, ksize, num_threads, path_to_temp_dir):
    """
    ...
    :param manifest: a dataframe with the following columns:
                        ...
                        'sample_scale_factor',
                        'min_coverage'
    :param sample_file: path to the sample signature file
    ...
    # run the sourmash multisearch
    cmd = f"sourmash scripts multisearch {sample_sig_file_path} {organism_sig_file_path} -k {ksize} -c {num_threads} -t 0 -o {os.path.join(path_to_temp_dir, 'sample_multisearch_result.csv')}"
...
def get_exclusive_hashes(manifest, nontrivial_organism_names, sample_sig, ksize, num_threads, path_to_temp_dir):
    """
    ...
    def __find_exclusive_hashes(md5sum, path_to_temp_dir, ksize, single_occurrence_hashes):
        ...
        org_exclusive_hashes = {hash for hash in sig.minhash.hashes if hash in single_occurrence_hashes}
        return org_exclusive_hashes
    ...
    organism_names_list = sub_manifest['organism_name'].to_list()
    ...
    ## Use multithreading to speed up the process and reduce memory usage by sharing the variable single_occurrence_hashes
    with cf.ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = {executor.submit(__find_exclusive_hashes, md5sum, path_to_temp_dir, ksize, single_occurrence_hashes): md5sum for md5sum in tqdm(organism_md5sum_list, desc='Submitting jobs')}
        for future in tqdm(cf.as_completed(futures), total=len(futures)):
            exclusive_hashes_org.append(future.result())
...
def hypothesis_recovery(
    ...
    :param manifest: a dataframe with the following columns: 
                        ...
                        'sample_scale_factor',
                        'min_coverage'
    :param sample_info_set: a set of information about the sample, including the sample signature location, and the sample signature information
    :param path_to_temp_dir: path to the temporary directory generated by the training step
    :param ksize: k-mer size
    :param significance: significance level for the hypothesis test
    :param ani_thresh: threshold for ANI (i.e. how similar do the genomes need to be in order to be considered the same)
    ...
    sample_file, sample_sig, sample_sig_info = sample_info_set
    
    # Find the organisms that have non-zero overlap with the sample
    nontrivial_organism_names = get_organisms_with_nonzero_overlap(manifest, sample_file, ksize, num_threads, path_to_temp_dir)
    
    # Get the unique hashes exclusive to each of the organisms that have non-zero overlap with the sample
    exclusive_hashes_info, manifest = get_exclusive_hashes(manifest, nontrivial_organism_names, sample_sig, ksize, num_threads, path_to_temp_dir)
    ...
    new_manifest = pd.concat([manifest, results], axis=1)
