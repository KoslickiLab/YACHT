...
### Manual installation
YACHT requires Python 3 or higher. We recommend using a virtual environment (such as conda) to install YACHT. Before you begin the installation, you must also install [Rust](https://rustup.rs/). To create a virtual environment, run::
```bash
# Clone the repo
...
```bash
python run_YACHT.py --ref_matrix 'gtdb_ani_thresh_0.95_ref_matrix_processed.npz' --ksize 31 --sample_file 'sample.sig.zip' --ani_thresh 0.95 --significance 0.99 --min_coverage 1 --outfile 'yacht_results.csv'
```
