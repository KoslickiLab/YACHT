...
for ANI_threshold in ANI_thresholds:
    ...
    fp_array = np.zeros((len(coverage_thresholds), len(spike_coverages)))
    fn_array = np.zeros((len(coverage_thresholds), len(spike_coverages)))
    for coverage_threshold_index, coverage_threshold in enumerate(coverage_thresholds):
        for spike_coverage_index, spike_coverage in enumerate(spike_coverages):
            ...
            fp_array[coverage_threshold_index, spike_coverage_index] = df['FP']
            fn_array[coverage_threshold_index, spike_coverage_index] = df['FN']
    # round to 2 decimal places
    fp_array = np.round(fp_array, 2)
    fn_array = np.round(fn_array, 2)
    # make the heatmap
    ...
    ax[0].set_yticks(np.arange(len(spike_coverages)))
    ax[0].set_xticks(np.arange(len(coverage_thresholds)))
    ...
    ax[1].set_yticks(np.arange(len(spike_coverages)))
    ax[1].set_xticks(np.arange(len(coverage_thresholds)))
