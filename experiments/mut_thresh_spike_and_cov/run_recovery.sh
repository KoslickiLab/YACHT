		...
		do
			for line in `tail -n +2 sigs_md5_to_accession_to_gtdb_location.txt | head -n 10`
	        	do
                		...
                		echo "python ../../recover_abundance.py --ref_file ../formatted_db_ref_matrix_processed.npz --ksize 31 --sample_file spikes_cov_${spikeCov}/${md5short}_spiked_sample.sig --outfile EU_on_spikes_cov_${spikeCov}/${md5short}_${spikeCov}X_cov_thresh_${covThresh}X_mut_thresh_${mut}.csv --min_coverage ${covThresh}" --mut_thresh ${mut}
	        	done | parallel -j 50
		done
	done
done
