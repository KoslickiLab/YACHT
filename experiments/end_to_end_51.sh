...
sed -n 2p ../MANIFEST_51.csv > known_names_picklist.txt
cat known_names.txt | cut -d'.' -f1 | xargs -I{} grep {} ../MANIFEST_51.csv >> known_names_picklist.txt
...
sourmash sig extract --picklist known_names_picklist.txt:md5:md5 ../formatted_db_51.sig -o without_unknown_db.sig 
echo "Making the EU dictionary"
python ../../ref_matrix.py --ref_file without_unknown_db.sig  --ksize 51 --out_prefix default_EU_ #--max_thresh=10000
...
numUnknownReads=$(../calculate_unknown_percent.py -d . -r ../formatted_db_51.sig)
...
echo "${unknownPercent},${sourmashEstimateUnknown},${ourEstimate},${numReads}" >> ../all_results_51.txt
