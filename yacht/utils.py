...
def load_signature_with_ksize(filename: str, ksize: int) -> sourmash.SourmashSignature:
    ...
    if len(sketches[0].minhash.hashes) == 0:
        raise ValueError(f"Empty hash in signature. Please try sketch with '--scaled=1' or use alternative to sourmash.")
    if math.isnan(sketches[0].minhash.mean_abundance):
        raise ValueError(f"No mean abundance. Please try sketch with '--scaled=1' or use alternative to sourmash.")
    return sketches[0]
