...
def get_organisms_with_nonzero_overlap(
    ...
    sample_sig_file_path = os.path.join(path_to_sample_temp_dir, "sample_sig_file.tsv")
    ...
    organism_sig_file_path = os.path.join(
        path_to_sample_temp_dir, "organism_sig_file.tsv"
    )
    organism_sig_file.to_csv(organism_sig_file_path, header=False, index=False)

    # run algorithm to calculate similarity between sample and organisms
    total_sig_files = len(organism_sig_file[0])
    if total_sig_files <= num_genome_threshold:
        passes = 1
    else:
        passes = int(total_sig_files / num_genome_threshold) + 1
    cmd = f"{FILE_LOCATION}/yacht_run_compute_similarity {sample_sig_file_path} {organism_sig_file_path} {path_to_sample_temp_dir} -t {num_threads} -p {passes} -c 0 -C -o {path_to_sample_temp_dir}/sample_comparison_result.csv"
    
    logger.info(f"Running similarity algorithm with command: {cmd}")
    exit_code = os.system(cmd)
    if exit_code != 0:
        raise ValueError(f"Error running similarity algorithm with command: {cmd}")

    # remove all split comparison files ("*.txt") but only keep the combined ones
    for file in glob.glob(os.path.join(path_to_sample_temp_dir, "*.txt")):
        os.remove(file)

    # read the similarity algorithm result
    comparison_result = pd.read_csv(
        os.path.join(path_to_sample_temp_dir, "sample_comparison_result.csv"),
        sep=",",
        header=None,
    )
    comparison_result = comparison_result.drop_duplicates().reset_index(drop=True)

    return [manifest['organism_name'].to_list()[i] for i in comparison_result[1].to_list()]
